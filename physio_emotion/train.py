"""Train PhysioMultiChannelModel on DEAP (6 physiological channels) for VA prediction.

Usage:
    python -m physio_emotion.train
    python -m physio_emotion.train --epochs 100 --batch_size 128
"""
...
from .dataset import DEAPPhysioDataset, DEAPPhysioWithinSubjectDataset
from .models import PhysioMultiChannelModel, ccc_loss, combined_loss
...
def train_one_epoch(model, loader, optimizer, scheduler, device):
    ...
    for signals, y in loader:
        ...
        loss = combined_loss(model(signals), y)
        loss.backward()
        optimizer.step()
...
def evaluate(model, loader, device):
    ...
    for signals, y in loader:
        preds_all.append(model(signals.to(device)).cpu().numpy())
    ...
    for i, dim in enumerate(["valence", "arousal"]):
        ...
        metrics[dim] = {"ccc": ccc, "r": r, "rmse": rmse}
...
def main(args):
    ...
    print("Loading DEAP physiological datasets…")
    ds_train = DEAPPhysioDataset(TRAIN_SUBJECTS, args.data_dir, WINDOW_SAMPLES, HOP_SAMPLES, augment=True)
    ds_val   = DEAPPhysioDataset(VAL_SUBJECTS,   args.data_dir, WINDOW_SAMPLES, HOP_SAMPLES, augment=False)
    ...
    model = PhysioMultiChannelModel().to(device)
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"PhysioMultiChannelModel: {n_params:,} trainable parameters")
    ...
    weights_path = os.path.join(args.cache_dir, "physio_va_deap.pt")
    ...
    axes[0].set_title("Physio → VA: Loss"); axes[0].legend()
    ...
    axes[1].set_title("Physio → VA: Validation CCC"); axes[1].legend()
    fig.tight_layout()
...
# ---------------------------------------------------------------------------
# Within-subject physio training
# ---------------------------------------------------------------------------

def _physio_train_epochs(model, train_loader, n_epochs, lr, weight_decay, patience_max, device):
    """Train physio model for up to n_epochs with train-loss early stopping."""
...
def train_within_subject_single(subject_id, args, device):
    """Train one per-subject PhysioMultiChannelModel and return its test metrics.
    ...
    """
    ds_train = DEAPPhysioWithinSubjectDataset(
        subject_id, "train", args.data_dir, WINDOW_SAMPLES, HOP_SAMPLES
    )
    ds_test = DEAPPhysioWithinSubjectDataset(
    ...
    train_loader = DataLoader(ds_train, batch_size=args.ws_batch_size, shuffle=True,  num_workers=2, pin_memory=True)
    test_loader  = DataLoader(ds_test,  batch_size=args.ws_batch_size, shuffle=False, num_workers=2, pin_memory=True)

    # Step 1: evaluate on 80/20 split
    model = PhysioMultiChannelModel().to(device)
    _physio_train_epochs(
        model, train_loader, args.ws_epochs, args.ws_lr, args.ws_weight_decay,
        args.ws_patience, device
    )
    _, test_m = evaluate(model, test_loader, device)

    # Step 2: train on ALL 40 trials → best deployed checkpoint
    ds_all    = DEAPPhysioWithinSubjectDataset(
        subject_id, "all", args.data_dir, WINDOW_SAMPLES, HOP_SAMPLES
    )
    all_loader = DataLoader(ds_all, batch_size=args.ws_batch_size, shuffle=True, num_workers=2, pin_memory=True)
    model_all  = PhysioMultiChannelModel().to(device)
    _physio_train_epochs(
        model_all, all_loader, args.ws_epochs, args.ws_lr, args.ws_weight_decay,
        args.ws_patience, device
    )

    ckpt_path = os.path.join(args.cache_dir, f"physio_ws_s{subject_id:02d}.pt")
...
def train_within_subject(args):
    """Train per-subject PhysioMultiChannelModel for all 32 DEAP subjects."""
    set_seed()
    device = torch.device(args.device if torch.cuda.is_available() else "cpu")
    os.makedirs(args.cache_dir, exist_ok=True)

    print(f"Within-subject physio training on all {N_SUBJECTS} subjects…")
    ...
    for sid in range(1, N_SUBJECTS + 1):
        ...
        a_ccc    = m["arousal"]["ccc"]
        print(
            f"  s{sid:02d} | V-CCC={v_ccc:+.4f}  A-CCC={a_ccc:+.4f} | {time.time()-t0:.1f}s"
        )
    ...
    print(f"Checkpoints saved to: {args.cache_dir}/physio_ws_s{{01..32}}.pt")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train physio emotion models on DEAP")
    parser.add_argument("--mode",         choices=["cross", "within_subject"], default="within_subject",
                        help="'cross' = cross-subject training; 'within_subject' = per-subject models")
