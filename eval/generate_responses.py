#!/usr/bin/env python3
"""
Generate audio responses from 3 model conditions for empathy evaluation.
...
Usage:
    conda run -n glm4voice3_eval python -m eval.generate_responses \\
        --num-samples 10 \\
        --output-dir results/eval_judge/

    # Use a specific checkpoint step:
    conda run -n glm4voice3_eval python -m eval.generate_responses \\
        --checkpoint-step 700 \\
        --output-dir results/eval_judge_ckpt700/

    # Quick test with 2 emotions:
    conda run -n glm4voice3_eval python -m eval.generate_responses \\
        --emotions happy sad \\
        --num-samples 2 \\
        --output-dir results/eval_judge_test/
"""
...
DEFAULT_FINETUNED_EXPERIMENT = (
    "experiments/glm-model-opens2s-qwen3tts-va-text-lora-20260226-124735"
)
DEFAULT_CHECKPOINT_STEP = 1400
DEFAULT_EVAL_AUDIO_DIR = "/engram/naplab/users/sd3705/Datasets/Sympatheia_11Emo_17k/audio/eval"
DEFAULT_OUTPUT_DIR = "/engram/naplab/users/sd3705/emo_recog_2025s/eval"
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=f"Output directory for audio files and manifest.jsonl. Default: {DEFAULT_OUTPUT_DIR}",
    )
    parser.add_argument(
