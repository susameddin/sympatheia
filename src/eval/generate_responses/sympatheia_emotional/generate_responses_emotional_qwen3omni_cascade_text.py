#!/usr/bin/env python3
"""
Stage 1 of the cascaded baseline: input audio → Qwen3-Omni → response text + talk style.
...
Usage:
    conda run -n qwen3omni --no-capture-output \\
        python -m eval.generate_responses.sympatheia_emotional.generate_responses_emotional_qwen3omni_cascade_text \\
        --manifest <path/to/sample_emotional_manifest.jsonl> \\
...
SYSTEM_PROMPT = (
    "You are a warm, empathetic voice assistant. Listen to the user and respond supportively, "
    "acknowledging their emotional state with genuine care.\n"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Stage 1 cascaded baseline: Qwen3-Omni → text + talk style"
    )
...
def parse_style_response(raw: str) -> tuple[str, str]:
    ...
    for i, line in enumerate(lines):
        if line.startswith("STYLE:"):
            ...
            # Everything from RESPONSE: onward (may span multiple lines)
