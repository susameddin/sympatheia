#!/usr/bin/env python3
"""
Generate audio responses from Kimi-Audio for empathy evaluation.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate Kimi-Audio responses for empathy evaluation"
    )
    parser.add_argument(
...
def load_model(model_path: str):
    # Add Kimi-Audio repo to path so kimia_infer imports resolve
...
def generate_response(model, query_audio_path: str) -> tuple[object, str]:
    """Run Kimi-Audio on one query and return (wav_tensor, text)."""
    messages = [
        {
            "role": "user",
            "message_type": "text",
            "content": "Please respond warmly and acknowledge my emotional state. Please respond in English.",
        },
...
def main():
    ...
    if todo:
        ...
        for idx, rec in enumerate(todo):
            ...
            t0 = time.time()
            try:
                wav_tensor, text_out = generate_response(model, query_audio)
