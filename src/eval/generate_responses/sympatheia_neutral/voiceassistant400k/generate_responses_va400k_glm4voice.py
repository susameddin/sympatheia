#!/usr/bin/env python3
"""
Generate base GLM-4-Voice responses on VoiceAssistant-400K queries.

Reads manifest.jsonl produced by generate_responses_va400k_sympatheia.py,
reuses the saved query WAVs, and generates base model responses. Writes
...
BASE_SYSTEM_PROMPT = "Please respond in English."
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate base GLM-4-Voice responses on VoiceAssistant-400K queries"
    )
    parser.add_argument(
        "--manifest", type=str, required=True,
        help="Path to manifest.jsonl from generate_responses_va400k_sympatheia.py",
    )
    parser.add_argument(
...
def main():
    ...
    todo = [
        ...
        if not (
            args.skip_existing
            and rec["id"] in existing_ids
            and (audio_dir / f"{rec['id']}.wav").exists()
        )
    ]
    ...
    if todo:
        ...
        for idx, rec in enumerate(todo):
            ...
            prompt = build_prompt(user_tokens, BASE_SYSTEM_PROMPT)
            ...
            if waveform is not None:
                ...
                print(f"  Saved: {out_wav.name}  ({elapsed:.1f}s)")
            else:
    ...
    # Write output manifest — preserves all finetuned fields, adds base_* fields
    ...
    total = len(records)
    eval_jsonl = manifest_path.parent / "eval.jsonl"
