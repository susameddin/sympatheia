#!/usr/bin/env python3
"""
Generate audio responses from Qwen3-Omni using a user text description + query audio.

Reads the text sensing manifest (produced by generate_responses_sensing.py --modality text),
which contains input_text (ISEAR self-reported emotion description) and query_audio per sample.
The text is passed directly to Qwen3-Omni so the model can read the user's own words and
respond with appropriate empathy — no intermediate VA extraction step.
...
CONDITION        = "text_qwen3omni"

def get_system_prompt(input_text: str) -> str:
    return (
        "You are a warm, empathetic voice assistant. "
        "The following is a description of what the user has been experiencing emotionally: "
        f"\"{input_text}\" "
        "Use this to understand how the user is feeling before you listen to their voice message. "
        "Your most important job is to acknowledge and directly address their emotional state — "
        "do not ignore it or give a generic response. "
        "If they are going through something sad or painful, lead with heartfelt consolation and let them know you care. "
        "If they are feeling angry or frustrated, respond calmly, validate their feelings, and show understanding. "
        "If they are feeling fearful or anxious, be gentle and reassuring. "
        "If they are feeling happy or excited, match their energy with warmth and enthusiasm. "
        "Always name or reflect their emotional state early in your response so they feel truly heard. "
        "Respond to their voice message — not to the description above. "
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate Qwen3-Omni responses conditioned on user text description + audio"
    )
    parser.add_argument(
        "--manifest", type=str, default=DEFAULT_MANIFEST,
        help=f"Path to text sensing manifest.jsonl (contains input_text and query_audio). "
    ...
    parser.add_argument(
        ...
        help="Comma-separated list of emotions to process, e.g. 'joy,sadness,fear'. "
...
def build_conversation(input_text: str, query_audio_path: str) -> list:
    """Build conversation with ISEAR context in system prompt; user turn is audio only."""
    return [
        {
            "role": "system",
            "content": [{"type": "text", "text": get_system_prompt(input_text)}],
        },
        {
            "role": "user",
            "content": [
...
def main():
    ...
    if todo:
        ...
        for idx, rec in enumerate(todo):
            ...
            print(f"  Text: {input_text[:100]!r}{'...' if len(input_text) > 100 else ''}")

            if not input_text:
                print(f"  SKIP: input_text is empty")
                continue
            if not query_audio or not Path(query_audio).exists():
                print(f"  SKIP: query audio not found: {query_audio!r}")
                continue

            conversation = build_conversation(input_text, query_audio)
            ...
            if text_out:
                print(f"  Response: {text_out[:120]!r}")
