...
Prereqs:
    # 1. Precompute HSEmotion face predictions (in 's' env):
    conda run -n s --no-capture-output python -m integration.face_module.precompute_hsemotion --n-per-class 150

    # 2. Generate face_va / no_va Sympatheia responses (in 'glm4voice3' env):
    conda run -n glm4voice3 --no-capture-output python -m eval.generate_responses.sensing.generate_responses_sensing --modality face

Usage:
    conda run -n qwen3omni --no-capture-output python -m eval.generate_responses.sensing.generate_responses_face_qwen3omni \\
        --manifest /engram/naplab/users/sd3705/emo_recog_2025s/eval/eval_face_hsemo/manifest.jsonl
...
DEFAULT_MANIFEST = "/engram/naplab/users/sd3705/emo_recog_2025s/eval/eval_face_hsemo/manifest.jsonl"
